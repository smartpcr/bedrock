resource "null_resource" "deploy_stored_procedures" {
  ...
  provisioner "local-exec" {
    command = "pwsh ${path.module}/ensure_cosmosdb_sp.ps1 -AccountName ${var.cosmos_db_account} -SubscriptionId ${var.cosmosdb_subscription_id} -DbSettings ${var.cosmosdb_settings} -VaultName ${var.vault_name}"
  }
  ...
  triggers = {
    ...
    cosmosdb_settings        = "${var.cosmosdb_settings}"
  }
}
